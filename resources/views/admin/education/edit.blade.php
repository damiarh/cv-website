...
<div>
    <div class="mb-6">
        <h1 class="text-3xl font-bold text-gray-800">Edit Education</h1>
        <p class="text-gray-600 mt-1">Update education details</p>
    </div>
        ...
        <form action="{{ route('admin.education.update', $education) }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="date" name="start_date" value="{{ old('start_date', $education->start_date->format('Y-m-d')) }}" 
                           class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent transition" 
                           required>
                    @error('start_date')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="description" rows="4" 
                              class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent transition" 
                              placeholder="Key achievements, coursework, GPA, etc.">{{ old('description', $education->description) }}</textarea>
                    @error('description')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
