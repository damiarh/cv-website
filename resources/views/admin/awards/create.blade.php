...
<div>
    <div class="mb-6">
        <h1 class="text-3xl font-bold text-gray-800">Add Award</h1>
        <p class="text-gray-600 mt-1">Add a new achievement or honour</p>
    </div>
        ...
        <form action="{{ route('admin.awards.store') }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="date" name="date_received" value="{{ old('date_received') }}" 
                           class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-yellow-500 focus:border-transparent transition" 
                           required>
                    @error('date_received')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="description" rows="4" 
                              class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-yellow-500 focus:border-transparent transition" 
                              placeholder="Details about the award, why you received it, etc.">{{ old('description') }}</textarea>
                    @error('description')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
